...
def restaurantMenuByIdJSON(restaurant_id, menu_id):
    restaurant = session.query(Restaurant).filter_by(id=restaurant_id).one()
    items = session.query(MenuItem).filter_by(
        id=menu_id).one()
...
def editMenuItem(restaurant_id, menu_id):
    ...
    originialItem = session.query(MenuItem).filter_by(id=menu_id).one()
    if request.method == 'POST':
        if request.form['name']:
            editedItem.name = request.form['name']
        session.add(editedItem)
        session.commit()
        flash('Item ' + originialItem.name + ' successfully edited become ' + editedItem.name)
        return redirect(url_for('restaurantMenu', restaurant_id=restaurant_id))
    else:
...
def deleteMenuItem(restaurant_id, menu_id):
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash('Item ' + itemToDelete.name +' successfully deleted')
        return redirect(url_for('restaurantMenu', restaurant_id=restaurant_id))
    else:
